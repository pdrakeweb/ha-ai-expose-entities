...
- **`entity/`** - Base entity classes (inherit `AIExposeEntitiesEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, AIExposeEntitiesEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import AIExposeEntitiesEntity`
