...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `AIExposeEntities`
...
- Inherit from both platform entity and `AIExposeEntitiesEntity` (order matters)
...
from custom_components.ai_expose_entities.sensor import async_setup_entry
...
**Config entry data:** `entry_data: AIExposeEntitiesData = hass.data[DOMAIN][entry.entry_id]`
