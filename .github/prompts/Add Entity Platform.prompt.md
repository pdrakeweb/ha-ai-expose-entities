...
**Directory:** `custom_components/ai_expose_entities/[platform]/`
...
```python
"""[Platform] platform for AI Expose Entities."""
...
from .[entity_file] import AIExposeEntities[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: AIExposeEntitiesDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            AIExposeEntities[EntityName](coordinator, entry),
...
```python
"""[Entity description] for AI Expose Entities."""
...
class AIExposeEntities[EntityName](
...
Add platform to `custom_components/ai_expose_entities/manifest.json`:
...
- [ ] Entity class inherits from both `AIExposeEntitiesEntity` and platform class
...
- **Base entity:** `AIExposeEntitiesEntity` in `entity/base.py`
- **Coordinator:** `AIExposeEntitiesDataUpdateCoordinator`
